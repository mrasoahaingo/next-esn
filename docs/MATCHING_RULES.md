...
- **Historique d’analyse** : snapshots versionnés (`kind: positioning_analysis_snapshot`, v1) dans **`ai_usage_log.output_payload`** avec `task_key = positioning.analysis.snapshot` — enregistrés à la **fin d’un workflow d’analyse réussi** et **avant** une relance / repositionnement (archivage). Même source que le journal LLM admin ; index partiel `idx_ai_usage_log_positioning_snapshot`. Migration : `20260413_positioning_history_ai_usage_log.sql` (remplace l’ancienne table dédiée).
...
- **Verdict final (synthèse)** : le message utilisateur de la synthèse (`buildPositioningSynthesisUserContent` dans `lib/services/positioning.service.ts`) impose de ne fonder **matchScore**, **matchSummary**, **matchScoreConfidence** et **matchScoreConfidenceNote** que sur le parcours pertinent des **5 dernières années** par rapport à la date de référence (UTC) ; le parcours plus ancien ne doit pas faire varier le score ni la confiance (mention contextuelle au plus une phrase dans le résumé si besoin).
- **Niveau (Lead / Senior / Expert, etc.) — synthèse** : pour le verdict, ne pas utiliser au-delà des **deux expériences les plus récentes** (`experiences[0]` et `experiences[1]`, 0 = plus récent). **`experiences[0]`** = niveau **actuel** (référence principale) ; **`experiences[1]`** = **corroboration** du poste précédent. Une **ambiguïté** dans la fiche / barème **ne doit pas** être le **motif principal** d’une baisse de **matchScoreConfidence** lorsque le niveau démontré sur le poste actuel (et cohérent avec le précédent) est aligné avec le besoin.
